...
This directory contains the infrastructure for validating Manim datasets and preparing them for training a code generation model.

## Overview

Layer 4 implements a three-phase training pipeline:
1. **DAPT (Domain Adaptive Pre-Training)**: Pure Manim code to build API fluency
2. **Step-level SFT**: Single-transformation instructions → code
3. **Scene-level SFT**: High-level descriptions → complete scenes
